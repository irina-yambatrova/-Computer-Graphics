...
{
	const glm::vec4 WHITE = { 0.99f, 0.99f, 0.99f, 0.99f };
	GetWindow().SetBackgroundColor(WHITE);
	CheckOpenGLVersion();
	SetupOpenGLState();

	const std::string vertexShader = CFilesystemUtils::LoadFileAsString("res/copytexture.vert");
	const std::string pictureShader = CFilesystemUtils::LoadFileAsString("res/ring.frag");
	...
	m_programQueue = { &m_programPicture};
}
